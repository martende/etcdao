...
func processNode(node *client.Node, v interface{}, tag reflect.StructTag) error {
	...
	case *int:
		*v, err = strconv.Atoi(node.Value)
		if DEBUG && err != nil {
			log.Printf("processNode: node.Value is not int '%v'", node.Value)
		}
	case *string:
		*v = node.Value
	case *time.Time:
		...
		*v, err = time.Parse(format, node.Value)
		if DEBUG && err != nil {
			log.Printf("processNode: node.Value is not time '%v' for format '%s' err=%v", node.Value, format, err)
		}
	case *bool:
	...
	default:
		...
		case reflect.Map:
			if DEBUG && !node.Dir {
				log.Printf("processNode: type %v needs dir in etcd , not scalar", reflect.TypeOf(v))
				return ErrBadFormat
			}
		...
		case reflect.Slice:
			...
			for _, v := range node.Nodes {
				...
				if DEBUG && err != nil {
					log.Printf("processNode: dir element '%s' must be integer", v.Key[li+1:])
					return ErrBadFormat
				}
		...
		default:
			if DEBUG {
				log.Printf("processNode: Unkonwn type %v KIND=%v", reflect.TypeOf(v), tp.Kind())
			}
			return ErrBadFormat
		}
	}
...
*/
func ReadObject(kapi client.KeysAPI, ctxt context.Context, path string, v interface{}) error {
